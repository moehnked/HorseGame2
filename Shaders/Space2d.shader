...
uniform float zoom = 50.01;
uniform float tile = 0.250;
uniform float speed = 0.00002;

uniform float brightness = 0.001;
uniform float darkmatter = 1.9500;
uniform float distfading = 0.730;
uniform float saturation = 0.950;
uniform vec2 projectResolution = vec2(1024,600);
...
void fragment()
{
	...
	COLOR = vec4(fragColor.rgb + -vec3(0.15,0.05,-0.01),finalAlpha);
	//ALPHA = fragColor.a;
